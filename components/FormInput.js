const FormInput = ({ labelname, placeholder, handleChange }) => {
    return (
        <div className="pt-4 flex flex-col w-4/5">
            <label className="pb-2">{labelname}</label>
            <input type="number" placeholder={placeholder} onChange={handleChange} className="w-full h-12 rounded-md p-4 bg-gray-200" defaultValue="£"></input>
        </div>
    );
}
